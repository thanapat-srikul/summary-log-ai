...
export function evaluateRules(window: HostWindow, event: ConnEvent): RuleResult {
  ...
  const add = (points: number, code: string, reason: string, action: string) => {
    score += points;
    codes.push(code);
    reasons.push(reason);
    actions.add(action);
  ...
  if (window.connectionCount >= 100) add(25, "HIGH_CONNECTION_RATE", "มีการเชื่อมต่ออย่างน้อย 100 ครั้งต่อนาที", "ตรวจสอบ process และปลายทางที่อุปกรณ์ติดต่อ");
  ...
  if (window.failedCount >= 10 && window.failedCount / Math.max(1, window.connectionCount) >= 0.5) add(30, "HIGH_FAILURE_RATIO", "การเชื่อมต่อล้มเหลวอย่างน้อย 10 ครั้งและมากกว่าครึ่งหนึ่ง", "ตรวจสอบ authentication และ firewall log");
  if (window.byteCount >= 100 * 1024 * 1024) add(25, "HIGH_TRANSFER_VOLUME", "ถ่ายโอนข้อมูลรวมอย่างน้อย 100 MB ต่อนาที", "ยืนยันชนิดข้อมูลและปลายทางที่รับส่งข้อมูล");
  if (window.uncommonPortCount > 0 || !standardPorts.has(event.resp_p)) add(10, "UNCOMMON_PORT", "พบการเชื่อมต่อผ่านพอร์ตนอกกลุ่มมาตรฐาน", "ยืนยันว่าบริการบนพอร์ตดังกล่าวได้รับอนุญาต");
  ...
  if (window.maxFlowBytes >= 50 * 1024 * 1024) add(20, "LARGE_SINGLE_FLOW", "Flow เดียวมีข้อมูลอย่างน้อย 50 MB", "ตรวจสอบไฟล์หรือข้อมูลที่ถูกถ่ายโอน");
